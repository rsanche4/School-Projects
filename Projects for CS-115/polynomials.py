...
# Favorite flavor of dorito : Vanilla
# *****
...
class Term:
    ...
    def __call__(self, input): # myTerm(input)
        ...
        poweredx = input ** self.exp
        multiplyByK = self.coef * poweredx
        output = multiplyByK
        return output
    ...
    def __neg__(self): # otherTerm = -myTerm
        ...
        negativeK = self.coef * -1
        return Term(negativeK, self.exp)
...
# exponent is strictly decreasing
    ...
    def __eq__(self, other): # myPoly == other
        ...
        if Term(self.polyList[0]) == Term(other.polyList[0]) and \
           Term(self.polyList[1]) == Term(other.polyList[1]):
            return True
        else:
            return False
    ...
    def __call__(self, input): # myPoly(input)
        ...
        for i in range(0, len(self.polyList)):
            theTerm = self.polyList[i]
            result = theTerm(input)
            NewListForEachTerm.append(result)
        return sum(NewListForEachTerm)
