 ...
 * Description : Performs all basic methods as well as other metrics for a Red Black Tree.
...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        // TODO
        iterator dup = find(key);
        if (dup.node_ptr == nullptr) {
     ...
     */
    size_t size() const {
    	size_t size = leaf_count() + internal_node_count();
        return size;
    }
    ...
    double successful_search_cost() const {
        return size() == 0 ? 0 : 1 + (double)sum_levels() / size();
    }
...
private:
    ...
    int diameter(Node<K, V> *node) const {
    	...
    	int right_h = height(node->right);
       	...
       	if (left_h + right_h > std::max(left_d, right_d)) {
       		return left_h + right_h;
       	} else {
       		return std::max(left_d, right_d);
       	}
    }
