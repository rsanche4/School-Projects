...
(define (indegree Es v)
  (if (null? Es) 0
      (cond
        ...
        [(not (equal? (car (cdr (car Es))) v)) (indegree (cdr Es) v)])))
...
(define (find-top G)
  (cond
    [(null? (get-vertices G)) (tsort-error)]
    [(equal? (indegree (get-edges G) (car (car G))) 0) (car (car G))]
    [(not (equal? (indegree (get-edges G) (car (car G))) 0)) (find-top (list (cdr (get-vertices G)) (get-edges G)))]
    ))
