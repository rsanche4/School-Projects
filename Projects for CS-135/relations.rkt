...
(define (compose RO RI)
  (define (helper-compose RO RI var) (if (null? RI) '() (if (null? RO)
      (helper-compose var (cdr RI) var)
      (if (equal? (car (cdr (car RI))) (car (car RO)))
                (make-set (append (list (list (car (car RI)) (car (cdr (car RO)))))
      (helper-compose (cdr RO) RI var))) (helper-compose (cdr RO) RI var)))))
 (helper-compose RO RI RO))
...
(define (power R k)
  (define (helper-power R helper k)
    (if (equal? k 0)
        '()
        (if (equal? k 1) helper (helper-power R (compose R helper) (- k 1)))))
  (helper-power R R k))
...
(define (transitive-closure R)
  (define (transitive-closure-helper R var)
    (if (equal? var 0) '()
        (if (equal? var 1) R
            (union (power R var) (transitive-closure-helper R (- var 1))))))
 (transitive-closure-helper R (cardinality R)))
...
(define (transitive? R) (if (null? R) #t (set-equal? R (transitive-closure R))))
...
(define (EQ-relation? R n)
  (if (and (reflexive? R n) (symmetric? R)
           (transitive? R)) #t
      #f))
