...
(define (encrypt plaintext key)
  (if (null? plaintext) '()
      (encrypt-helper (string->list plaintext) key)))

(define (encrypt-helper text secondKey)
  (cond [(null? text) '()]
        [else (append (list (mod-exp (char->integer (car text))
                                     (car (cdr secondKey)) (car secondKey)))
        (encrypt-helper (cdr text) secondKey))]))
...
(define (decrypt ciphertext key)
  (if
   (null? ciphertext) '()
   (list->string (decrypt-helper ciphertext key))))

(define (decrypt-helper text2 secondKey)
  (if
   (null? text2) '()
   (append (list (integer->char (mod-exp (car text2) (car (cdr secondKey)) (car secondKey))))
           (decrypt-helper (cdr text2) secondKey))))
